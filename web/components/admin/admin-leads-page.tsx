...
/**
 * 组件：AdminLeadsPage
 * 用途：后台联系线索管理页，接入列表与状态流转。
 */
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { ChevronLeft, ChevronRight, Eye, Loader2, X } from "lucide-react";
import { toast } from "sonner";
import { getAdminLeads, updateAdminLeadStatus } from "@/lib/api/admin-leads";
...
function getLeadStatusLabel(status: LeadStatus): string {
  return (
    LEAD_STATUS_OPTIONS.find((option) => option.value === status)?.label ?? status
  );
}
...
function leadStatusBadgeClass(status: LeadStatus): string {
  switch (status) {
    ...
    case "won":
      return "border-lime-400/15 bg-lime-300/10 text-lime-200";
    case "lost":
...
export function AdminLeadsPage() {
  ...
  const [list, setList] = useState<AdminLead[]>([]);
  const [total, setTotal] = useState(0);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [detail, setDetail] = useState<AdminLead | null>(null);
  const [draftStatuses, setDraftStatuses] = useState<Record<number, LeadStatus>>({});
  ...
  const emptyMessage = useMemo(() => {
    ...
    if (!loading && list.length === 0) return "当前筛选条件下暂无联系线索。";
    return null;
  ...
  const loadList = useCallback(async () => {
    ...
    try {
      ...
      if (reqId !== requestIdRef.current) return;
      const message =
        e instanceof ApiError ? e.message : "联系线索加载失败，请稍后重试。";
      setList([]);
      setTotal(0);
      setError(message);
      toast.error(message);
  ...
  const handleUpdateStatus = useCallback(
    async (item: AdminLead) => {
      ...
      try {
        ...
        toast.error(e instanceof ApiError ? e.message : "线索状态更新失败，请稍后重试。");
      } finally {
        setPendingId(null);
      }
    },
      ...
      <section className="rounded-[28px] border border-white/10 bg-[#121212] p-6">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-semibold">联系线索</h1>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-white/55">
              当前页已接入 <code>/api/admin/leads</code>，支持线索列表、状态筛选、详情查看与状态流转。
            </p>
          </div>
      ...
      <section className="overflow-hidden rounded-[28px] border border-white/10 bg-[#121212]">
        <div className="flex items-center justify-between gap-3 border-b border-white/8 px-6 py-4">
          <div className="text-sm text-white/72">
            共 <span className="font-medium text-white">{total}</span> 条联系线索
          </div>
          ...
          <table className="min-w-[1120px] w-full">
            <thead className="bg-black/20">
              <tr className="text-left text-xs uppercase tracking-[0.16em] text-white/35">
                {["ID", "姓名", "联系方式", "公司", "需求类型/场景", "状态", "创建时间", "操作"].map((label) => (
                  ...
                  <tr key={item.id} className="border-t border-white/6 text-sm text-white/78">
                    ...
                    <td className="px-4 py-4 text-white/62">
                      <div className="max-w-[220px]">
                        <div className="truncate">{item.scene ?? "未填写场景"}</div>
                        <div className="mt-1 truncate text-xs text-white/42">
                          {stringifyJsonArray(item.dataTypes)}
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-4">
                      <span className={`inline-flex rounded-full border px-2.5 py-1 text-xs ${leadStatusBadgeClass(item.status)}`}>
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            setDraftStatuses((prev) => ({
                              ...prev,
                              [item.id]: event.target.value as LeadStatus,
                            }))
                          }
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h2 className="mt-3 text-2xl font-semibold">{detail.name}</h2>
              </div>
              <button
                type="button"
                onClick={() => setDetail(null)}
            ...
            <div className="mt-6 rounded-[24px] border border-white/10 bg-black/20 p-5">
              <div className="flex flex-wrap gap-2">
                <span className={`inline-flex rounded-full border px-2.5 py-1 text-xs ${leadStatusBadgeClass(detail.status)}`}>
                  {getLeadStatusLabel(detail.status)}
                </span>
              </div>

              <div className="mt-5">
                <DetailRow label="线索 ID" value={detail.id} />
                ...
                <DetailRow label="数据类型" value={stringifyJsonArray(detail.dataTypes)} />
                <DetailRow label="项目阶段" value={detail.stage ?? "-"} />
                <DetailRow label="预算" value={detail.budget ?? "-"} />
                <DetailRow label="创建时间" value={toDateTimeText(detail.createdAt)} />
                <DetailRow label="更新时间" value={toDateTimeText(detail.updatedAt)} />
                <DetailRow label="需求说明" value={detail.message || "-"} />
              </div>
            </div>
          </div>
        </div>
