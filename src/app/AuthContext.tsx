/**
 * 模块：全站登录态上下文 AuthContext
 * 用途：集中维护当前登录用户与登录态，供 NavBar 显示、AuthPage 写入、后续个人中心/上传/收藏读取。
 * 主要功能：
 *   1. 启动自举：挂载时若本地存在 token，则调用 GET /api/auth/me 恢复登录态（刷新不掉登录）。
 *   2. setAuth：登录/注册成功后保存 accessToken（写 localStorage）并设置当前用户。
 *   3. logout：调用 /api/auth/logout（容错）+ 清除本地 token + 清空用户。
 * 说明：token 读写复用 src/lib/token.ts；http.ts 在任意接口 401 时已自动 clearToken，这里兜底清空 user。
 */
...
interface AuthContextValue {
  ...
  bootstrapping: boolean; // 启动自举中（首次用 token 拉取 me 期间），用于避免 UI 闪烁
