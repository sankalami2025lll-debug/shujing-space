 ...
 * 主要功能：顶部导航、Hero 首屏、公司介绍、核心能力卡片、服务场景、愿景区、CTA 转化区、Footer
 * 对应文档：页面功能注释文档/10_关于我们_AboutUs.md
 */
import logoSrc from "../imports/____logo_1_.png";
import heroBg from "../imports/____-1.png";
import {
  ArrowRight,
  Box,
  Layers,
...
// AboutUs 页导航回调：由 App.tsx 注入，用于顶部导航与按钮跳转
interface AboutUsProps {
  onNavigateHome: () => void; // 返回首页
  onNavigateCommunity: () => void; // 进入模型社区
  onNavigateContact?: () => void; // 进入联系我们页（Hero / CTA / 导航联系按钮共用）
  onNavigateAuth?: () => void; // 进入注册登录页
}
...
function SpaceVisual() {
  return (
    <div className="relative w-full aspect-[4/3] md:aspect-[4/5] rounded-[22px] border border-white/10 overflow-hidden bg-gradient-to-br from-zinc-900 via-slate-900 to-zinc-950">
      ...
      <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse at 50% 35%, rgba(96,165,250,0.1) 0%, transparent 65%)" }} />
      ...
      <div className="absolute bottom-6 left-6 right-6 flex items-center justify-between">
        ...
        <div className="flex gap-1">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="h-0.5 rounded-full" style={{
              width: `${16 + i * 6}px`,
      ...
      <div className="relative w-[280px] h-[240px] md:w-[380px] md:h-[340px]">
        <svg width="100%" height="100%" viewBox="0 0 380 340" fill="none">
          ...
          <g fill="rgba(255,255,255,0.2)">
            {[[30, 150], [350, 80], [30, 280], [360, 260], [190, 290]].map(([cx, cy], i) => (
...
// AboutUs：公司介绍页主组件，整页为静态展示，无表单与数据请求
export default function AboutUs({ onNavigateHome, onNavigateCommunity, onNavigateContact, onNavigateAuth }: AboutUsProps) {
  // 站点配置：Footer 联系方式 / 公司名 / 版权 / 备案号来自后端（默认值兜底）
  const { config } = useSiteConfig();
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white overflow-x-hidden">

      {/* 顶部导航：当前页为关于我们，activePage 设为 about 以高亮对应导航项；onNavigateAbout 留空（已在本页无需跳转） */}
      <NavBar
        activePage="about"
      ...
      <section className="relative h-screen flex items-center overflow-hidden pt-16 md:pt-[72px]">
        <div className="absolute inset-0">
          <img src={heroBg} alt="空间数据" className="w-full h-full object-cover object-center" />
          ...
          <div className="grid grid-cols-1 items-center">
            <div>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
                {/* Hero 主按钮：平滑滚动到核心能力区锚点 #core-capabilities */}
                <button
                ...
                {/* Hero 次要按钮：跳转联系我们页（onNavigateContact 为可选，用可选链调用） */}
                <button
      ...
      <section className="relative py-16 md:py-32 bg-gradient-to-b from-[#0a0a0a] to-[#0d0d0d]">
        <div className="max-w-[1200px] mx-auto px-5 md:px-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-center">
            <div>
              <p className="text-[12px] md:text-[13px] text-gray-500 uppercase tracking-widest mb-3 md:mb-4 font-mono">关于数境空间</p>
              <h2 className="text-[26px] md:text-[38px] font-bold mb-2 md:mb-3 leading-tight">我们是谁</h2>
              <p className="text-[14px] md:text-[16px] text-gray-500 mb-6 md:mb-10 leading-relaxed">一家专注真实三维空间数据服务的科技公司</p>
              <div className="space-y-5 md:space-y-6 text-[14px] md:text-[16px] text-gray-400 leading-relaxed">
                <p>
                  数境空间（深圳）科技有限公司是一家专注<strong className="text-white font-semibold">真实三维空间数据服务</strong>的科技企业。
                </p>
                <p>
                  我们围绕建筑、园区、商业空间、文旅场景等真实空间对象，提供实景重建、BIM 模型处理、构件级模型、云端展示、数字孪生平台接入与具身智能空间训练数据服务。
                </p>
                <p>
                  通过 3DGS、倾斜摄影、实景 VR、BIM 与云端展示能力，服务于工程改造、数字文旅、游戏影视、数字存档、云上营销与具身智能空间训练等场景。
                </p>
              </div>
              <div className="mt-8 md:mt-10 flex flex-wrap gap-2">
                {["真实三维空间数据", "数字孪生", "实景三维", "BIM", "具身智能空间训练", "云端轻量化展示"].map(kw => (
                  <span key={kw} className="px-3 py-1.5 rounded-full border border-white/10 bg-white/[0.03] text-[12px] md:text-[13px] text-gray-300">{kw}</span>
                ))}
              </div>
            </div>
            <SpaceVisual />
        ...
        <div className="max-w-[1200px] mx-auto px-5 md:px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-5">
            {capabilities.map((cap, i) => (
              <div key={cap.title} className="group relative bg-white/[0.03] backdrop-blur-sm rounded-[22px] border border-white/10 p-6 md:p-7 hover:border-cyan-500/20 hover:bg-white/[0.045] transition-all duration-300">
                ...
                <h3 className="text-[16px] md:text-[17px] font-semibold mb-2 md:mb-3">{cap.title}</h3>
                <p className="text-[13px] md:text-[14px] text-gray-500 leading-relaxed">{cap.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="relative py-16 md:py-32 bg-gradient-to-b from-[#0d0d0d] to-[#0a0a0a]">
        <div className="max-w-[1200px] mx-auto px-5 md:px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-5">
            {scenarios.map(s => (
              <div key={s.title} className="group flex gap-4 md:gap-5 items-start p-5 md:p-6 rounded-[20px] border border-white/[0.08] bg-white/[0.02] hover:border-white/15 hover:bg-white/[0.035] transition-all duration-200">
                ...
                <div>
                  <h3 className="text-[16px] md:text-[17px] font-semibold mb-1.5 md:mb-2">{s.title}</h3>
                  <p className="text-[13px] md:text-[14px] text-gray-500 leading-relaxed">{s.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="relative py-20 md:py-40 overflow-hidden">
        ...
        <div className="relative max-w-[1200px] mx-auto px-5 md:px-6 text-center">
          <p className="text-[12px] md:text-[13px] text-gray-600 uppercase tracking-widest font-mono mb-5 md:mb-6">我们的愿景</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-0 md:bg-white/[0.08] md:rounded-[20px] md:overflow-hidden">
            {[
              ...
              <div key={i} className="bg-[#0c0c0c] border border-white/[0.08] md:border-0 rounded-[16px] md:rounded-none px-6 md:px-10 py-8 md:py-12 text-center hover:bg-white/[0.025] transition-colors duration-300">
                <div className="w-8 h-px bg-cyan-400/40 mx-auto mb-5 md:mb-6" />
                <h3 className="text-[18px] md:text-[22px] font-semibold mb-2 md:mb-3">{item.label}</h3>
                <p className="text-[13px] md:text-[14px] text-gray-500 leading-relaxed">{item.sub}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-[1200px] mx-auto px-5 md:px-6">
          <div className="relative rounded-[24px] border border-white/10 bg-gradient-to-br from-white/[0.03] to-transparent p-8 md:p-16 text-center overflow-hidden">
            ...
            <div className="relative z-10">
              <h2 className="text-[24px] md:text-[44px] font-bold mb-4 leading-tight">一起构建真实空间的数据价值</h2>
              ...
              {/* CTA 主按钮：跳转联系我们页发起合作咨询（onNavigateContact 为可选，用可选链调用） */}
              <button
                onClick={() => onNavigateContact?.()}
        ...
        <div className="max-w-[1200px] mx-auto px-5 md:px-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-12 mb-10 md:mb-12">
            ...
            <div>
              ...
              {/* Footer 导航：首页 / 模型社区为可点击跳转，关于我们为当前页用静态文本标识 */}
