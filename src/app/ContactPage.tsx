 ...
 * 页面用途：官网核心业务转化页，承接客户咨询、项目需求、训练数据服务申请与模型服务需求
 * 主要功能：Hero 定位、咨询表单（多字段 + 数据类型多选 + 项目阶段单选 + 提交成功态）、我们可以提供、联系方式、服务流程、服务方向、CTA、Footer
 * 对应文档：页面功能注释文档/11_联系我们_ContactPage.md
 * 说明：表单已接入后端——选项来自 GET /api/contact/options（失败回退本地默认选项），提交调用 POST /api/contact/leads；
 *       全部字段受控，提交具备 loading / success / error 三态（第 10E）。
 */
import { useState, useEffect } from "react";
import { ArrowRight, Scan, Layers, Box, Brain, Monitor, Network, CheckCircle2, Loader2 } from "lucide-react";
import { toast } from "sonner";
import NavBar from "./NavBar";
import { useSiteConfig } from "./SiteConfigContext";
import logoSrc from "../imports/____logo_1_.png";
// 联系我们接口：表单选项配置 + 线索提交（均为公开接口，游客可调用）
import { getContactOptions, createLead } from "../lib/api/contact";
import { ApiError } from "../lib/http";

// ContactPage 导航回调：由 App.tsx 注入，用于顶部导航与按钮跳转
interface ContactPageProps {
  onNavigateHome: () => void; // 返回首页
  onNavigateCommunity: () => void; // 进入模型社区
  onNavigateAbout: () => void; // 进入关于我们
  onNavigateAuth: () => void; // 进入注册登录页（顶部导航「注册/登录」按钮使用）
  onNavigateModels: () => void; // 进入模型库（Hero「查看模型库」按钮使用）
}

// 以下四组为「默认选项」：正常情况下选项来自后端 GET /api/contact/options，
//   接口失败时回退到这些数组保证表单可用；文案与后端 contact.constants.ts 逐字对齐。
// DEFAULT_SCENE_OPTIONS：业务场景下拉默认选项，对应未来数据库字段（如 scene）
...
// DEFAULT_DATA_TYPE_OPTIONS：所需数据类型多选标签默认选项，未来对应数据库字段（如 data_types 数组）
...
// DEFAULT_STAGE_OPTIONS：项目阶段单选标签默认选项，未来对应数据库字段（如 project_stage）
...
// DEFAULT_BUDGET_OPTIONS：预算范围下拉默认选项，对应未来数据库字段（如 budget）
...
// processSteps：服务流程四步展示数据（需求沟通 → 方案确认 → 数据处理 → 交付接入）
const processSteps = [
  {
    num: "01",
    title: "需求沟通",
...
// serviceDirections：服务方向卡片数据，展示围绕真实三维空间数据的各业务方向
...
export default function ContactPage({
  ...
  // 站点配置：侧栏 / Footer 联系方式 / 公司名 / 版权 / 备案号来自后端（默认值兜底）
  const { config } = useSiteConfig();

  // submitted：表单是否提交成功，为真时显示成功态，为假时显示表单（由 POST /api/contact/leads 成功后置真）
  const [submitted, setSubmitted] = useState(false);
  // submitting：提交请求进行中标志，用于按钮 loading 态与防连点
  const [submitting, setSubmitting] = useState(false);

  // ── 受控文本字段（对应 CreateLeadDto：name/contactWay/company/email/message）──
  const [name, setName] = useState(""); // 姓名（必填，对应 contact_leads.name）
  const [contactWay, setContactWay] = useState(""); // 手机 / 微信（必填，对应 contact_way）
  const [company, setCompany] = useState(""); // 公司名称（可选）
  const [email, setEmail] = useState(""); // 联系邮箱（可选，填写须合法）
  const [message, setMessage] = useState(""); // 项目需求描述（可选，对应 message）

  // selectedDataTypes：用户选择的所需数据类型，可多选（对应 dataTypes）
  const [selectedDataTypes, setSelectedDataTypes] = useState<string[]>([]);
  // selectedStage：项目阶段单选状态（对应 stage）
  const [selectedStage, setSelectedStage] = useState("");
  // selectedScene：业务场景下拉选择状态（对应 scene）
  const [selectedScene, setSelectedScene] = useState("");
  // selectedBudget：预算范围下拉选择状态（对应 budget）
  const [selectedBudget, setSelectedBudget] = useState("");

  // ── 表单选项（默认用本地常量，挂载后用 GET /api/contact/options 覆盖；接口失败则保留默认）──
  ...
  // 挂载时拉取后端表单选项；成功则覆盖四组选项，失败静默回退本地默认（不打断页面、不弹错）
  useEffect(() => {
    getContactOptions()
      .then((opts) => {
      ...
      .catch(() => {
        // 选项接口失败：保留本地默认选项，不影响表单使用
      });
  }, []);

  // toggleDataType：切换某个数据类型的选中状态（已选则移除，未选则加入），由数据类型标签按钮触发
  ...
  // resetForm：成功态点「返回表单」后清空所有字段，避免保留上次输入
  const resetForm = () => {
    setName("");
    setContactWay("");
    setCompany("");
  ...
  // handleSubmit：提交客户咨询线索。前端先做必填校验，再调用 POST /api/contact/leads，三态处理。
  const handleSubmit = async () => {
    if (submitting) return;
    // 必填校验：姓名、手机/微信（与后端 CreateLeadDto 一致）
    if (!name.trim()) {
      toast.error("请填写姓名");
      return;
    }
    ...
    setSubmitting(true);
    try {
      // 组装入参：可选空字段不传（由后端存 null），避免空邮箱触发合法性校验
      await createLead({
        name: name.trim(),
      ...
      setSubmitted(true); // 成功后切换成功态
    } catch (e) {
      // 失败：用后端中文错误文案提示（如「email 必须为合法邮箱」），停留表单可重试
  ...
  // scrollToForm：平滑滚动到表单区域，由 Hero 与 CTA 的「提交需求」按钮触发
  const scrollToForm = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white overflow-x-hidden">
      {/* 顶部导航：当前为联系我们页，activePage 传 "contact"，主导航三项均不高亮，避免误高亮首页 */}
      {/* onNavigateContact 给空函数（已在本页无需跳转）；onNavigateAuth 转发以修复顶部「注册/登录」按钮原本无效的问题 */}
      <NavBar
        activePage="contact"
      ...
      {/* Hero：页面定位与服务咨询入口，提供「提交需求」（滚动到表单）与「查看模型库」两个 CTA */}
      ...
      {/* 咨询表单区：左侧为需求表单（submitted 为真时显示成功态），右侧为「我们可以提供」与联系方式侧栏 */}
      <section id="contact-form" className="py-16 md:py-24 bg-[#0d0d0d]">
        <div className="max-w-[1200px] mx-auto px-5 md:px-6">
          <div className="mb-10">
            <h2 className="text-[26px] md:text-[36px] font-bold mb-3">
              提交你的项目需求
            </h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-10">
            <div className="bg-white/[0.025] border border-white/10 rounded-[24px] p-6 md:p-8">
              {/* 提交成功态：显示成功提示与「返回表单」按钮（点击重置 submitted 回到表单） */}
              {submitted ? (
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      ...
                      {/* 姓名：受控必填字段，对应 CreateLeadDto.name */}
                      <input
                        type="text"
                        value={name}
                    ...
                    <div>
                      ...
                      {/* 手机 / 微信：受控必填字段，对应 CreateLeadDto.contactWay */}
                      <input
                        type="text"
                        value={contactWay}
                    ...
                    <div>
                      ...
                      {/* 公司名称：受控可选字段，对应 CreateLeadDto.company */}
                      <input
                        type="text"
                        value={company}
                    ...
                    <div>
                      ...
                      {/* 联系邮箱：受控可选字段，填写须合法，对应 CreateLeadDto.email */}
                      <input
                        type="email"
                        value={email}
                  ...
                  <div>
                    ...
                    >
                      ...
                      {/* 业务场景选项：来自后端 GET /api/contact/options（失败回退本地默认） */}
                  ...
                  <div>
                    <label className="block text-[13px] text-gray-400 mb-3">
                      所需数据类型
                    </label>
                    {/* 所需数据类型：多选标签，点击切换选中（选中态为冰蓝高亮） */}
                  ...
                  <div>
                    ...
                    {/* 项目需求描述：受控可选字段，对应 CreateLeadDto.message */}
                    <textarea
                      rows={4}
                      value={message}
                  ...
                  <div>
                    ...
                    >
                      ...
                      {/* 预算范围选项：来自后端 GET /api/contact/options（失败回退本地默认） */}
                  ...
                  {/* 提交需求：调用 POST /api/contact/leads；提交中禁用并显示 loading，成功切成功态，失败 toast */}
                  <button
                    type="button"
            ...
            {/* 侧栏：随页面滚动吸顶，包含「我们可以提供」服务清单与联系方式 */}
            <div className="sticky top-28 space-y-4 self-start">
              ...
              {/* 联系方式（侧栏）：电话、邮箱来自站点配置（GET /api/site-config），接口异常时回退默认占位「请填写」 */}
              <div className="bg-white/[0.025] border border-white/10 rounded-[20px] p-6">
      ...
      {/* Footer：底部信息区，含 Logo、联系方式占位与站内导航；联系方式待业务方提供真实信息后统一补全 */}
