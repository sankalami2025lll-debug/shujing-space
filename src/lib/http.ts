 ...
 * 主要功能：
 *   1. 统一读取 VITE_API_BASE_URL（缺省 /api，配合 vite dev proxy 代理到后端 4000）。
 ...
 * 说明：本步为基础设施，不接入任何具体页面业务。
 */
import { getToken, clearToken } from "./token";
import type { ApiResponse } from "./types";

// API_BASE_URL：后端 API 基址；优先取构建期注入的 VITE_API_BASE_URL，缺省回退 "/api"。
 ...
 * 失败时抛出 ApiError，由调用方或 useRequest 统一捕获处理三态。
 */
