...
|---|---|---|---|---|---|---|
| G1 | UI | 🔁 | 任意页面点击左上角整图 Logo | 返回首页 Home；顶部使用 `/loading/loading-logo-reference1.png`，高度为 60px | | |
...
| L8a | 🔌 | 🔁 | 有 `viewerUrl` 的模型 | 非 LCC 外链继续按原 viewer 行为展示；LCC/LCC2 模型详情页应挂载单个 iframe，地址为 `/viewer/lcc/[id]` | | |
| L8b | 🔌 | 🔁 | 无 `viewerUrl` 的模型 | 网格占位 UI，不报错 | | |
| L8c | UI | 🔁 | iframe 详情全屏/重置 | 全屏含 iframe；重置重新加载 | | |
| L8d | 🔌 | 🔁 | 打开任一 LCC 模型详情 | 外层品牌 Loading 在模型真正完成后关闭；iframe 内根节点应满足 `data-lcc-loaded=true`、`data-lcc-complete-reason=onLoadedStable`、`canvasCount>=1` | | |
| L8e | UI | 🔁 | 观察 LCC 模型底部右下角 | 当前内部版本允许底部做极小视觉裁切/底边遮挡；应避免明显补丁感，不影响模型交互 | | |
| L8f | UI | 🔁 | iframe 内展开「工具」→ 帮助 | 居中帮助浮层；仅「第一人称 / 枢轴」tab；无数字人/第三人称/新手指引 | | |
| L8g | UI | 🔁 | 帮助：`H` 开关 / `Esc` / 遮罩关闭 | 三种关闭方式可用；打开帮助时 walk 滚轮/鼠标/ WASD 不移动模型 | | |
| L8h | UI | 🔁 | 第一人称：滚轮 | 沿视线前后移动，视觉拉近/拉远，不改 FOV | | |
| L8i | UI | 🔁 | 第一人称：右键拖动 | 平移视角；不弹浏览器右键菜单；与左键转头不冲突 | | |
| L8j | UI | 🔁 | 枢轴模式 | OrbitControls 左旋转 / 滚轮缩放 / 右键平移正常；与 walk 切换无 pointer 残留 | | |
| L8k | UI | 🔁 | 模式文案 | 默认「第一人称」；orbit 侧为「枢轴」/「枢轴模式」 | | |
| L8l | 🔌 | 🔁 | 默认启动视图链 | 打开 / R 重置 / 保存启动视图后刷新，符合 launchView → sdkInitialCamera → … 优先级 | | |
